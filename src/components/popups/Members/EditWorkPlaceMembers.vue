...
<template>
  <div
    class="EditWorkPlaceMembers"
    v-for="(user, index) in userEmails"
    :key="index"
  >
    <p class="MemberUsername">{{ user.userEmail }}</p>
    <div class="MemberPrivilages">
      <button
        class="removeMember expandInput"
        @click="removeMember(user.collaboratorId, index)"
      >
        Remove
      </button>
    </div>
        ...
        if (response.status === 200) {
          ...
          collaborators.forEach((userInfo) => {
            this.userEmails.push({
              collaboratorId: userInfo.collaboratorId,
          ...
          const responseData = response.data;
          throw new Error(
            responseData.message || "Failed to fetch collaborators"
          );
        }
      } catch (error) {
